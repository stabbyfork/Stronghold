...
export default async (interaction: ChatInputCommandInteraction, args: typeof commandOptions.dpm.allies.remove) => {
	...
	if (
		...
		await interaction.reply({
			embeds: [
				defaultEmbed()
					.setTitle('Ally removed')
					.setDescription(`You are no longer allied with \`${tag}\`.`)
					.setColor('Green'),
			],
		});
	}
